...
const ADMIN_USER_ID = "53888547";
const ADMIN_DISPLAY_NAME = "BookSlumpRescueTeam";
...
async function upsertUser(claims: any) {
  ...
  if (!existingUser?.displayName) {
    if (isAdminUser) {
      userData.displayName = ADMIN_DISPLAY_NAME;
    } else if (claims["first_name"]) {
      const name = claims["last_name"]
